...
```
Adding `filter`/`lightpush`/`store` + pinned service nodes made `waku_new` **reject the config** → node reports offline. A relay node dials all bootstrap peers, joins the gossip mesh, and receives by mesh membership — no service node has to grant a filter lease (the fleet served "filter 0" unreliably). No `clusterId`/`shard` pinning — the preset + auto-sharding handle it. After `start`, wait ~10s for the mesh to form before the first publish. And **`entryNodes` must be a real, non-empty bootstrap list** — an empty `[]` (even with the preset set) leaves the node with no peers, the mobile face of "hub silently isolated." [^1]

**Core vs Edge — the battery/data knob, via the first-class `mode` field (NOT the light-client fields above).** `mode` accepts `"Core"` or `"Edge"`. `"Core"` is everything above — a full node that relays the shard and discovers peers. `"Edge"` is a *client-only* node: it publishes via **lightpush** and receives via **filter**, does NOT relay the shard, and doesn't run discv5 — so it's lighter on battery and cellular data but leans entirely on the fleet's Core nodes (and your own hub) to carry traffic. Crucially, `mode:"Edge"` is the **clean, config-accepted** way to get a light client — unlike hand-adding the raw `filter`/`lightpush` fields, which makes `waku_new` reject the config (§5 above). The Edge config drops `relay` and `discv5Discovery`: `{ mode:"Edge", preset, entryNodes, tcpPort:0 }`. [^12]

**Two rules for Edge on mobile.** (1) **`mode` is read only at node `start()`** — a runtime toggle must persist the choice and take effect on the next launch (or you tear down and recreate the node); a UI switch that just flips a variable does nothing until restart. (2) **Ship Core as the default and make Edge opt-in + tested on a real phone before defaulting.** Edge-on-cellular is the *exact* historically-flaky path from the "phone can't publish" saga (a light node couldn't relay-mesh; the real fix then was elsewhere, but the fragility was real) — so verify Edge actually meshes AND publishes on-device before rolling it out or flipping any other app to it. Desktop and the headless hub stay **Core** — they're the service nodes Edge clients depend on; an all-Edge topic has nobody relaying. [^12]

**The native bridge is CTX-BASED — thread the node handle through every call.** The JS surface is a lifecycle, not a global: `setup()` loads the libs (once, no args) → `ctx = await new(config)` creates a node and returns a **handle** → `start(ctx)` → then **every** call takes that `ctx` as its first arg: `subscribeContentTopic(ctx, topic)`, `channelCreate(ctx, id, topic, sender)`, `channelSend(ctx, id, json)`, `getNodeInfo(ctx, "Metrics")`, `storeQuery(ctx, …)`. Two failure modes if you get this wrong, and they only show on a real device: **never calling `new()`/`start()`** means no node exists; **passing the topic where `ctx` belongs** makes the Kotlin bridge do `BigInteger(topic).toLong()` → **`NumberFormatException` → a native crash on the first call** (e.g. on join). A ctx-less "global node" mental model is the trap — hold the `ctx` from `new()` in a module variable and pass it everywhere. [^1]
...
Ship: `expo prebuild --platform android` (re-applies the plugins — not optional), assert `app.json`'s `versionCode` matches the generated `build.gradle`, `gradlew assembleRelease -PreactNativeArchitectures=arm64-v8a`, then `fdroid update` on the **served** repo. Gotcha: publishing to the wrong repo dir (one with empty `metadata/`) silently produces an **empty index** — publish to the repo whose `metadata/<pkg>.yml` and fingerprint the phone already trusts. Version comes from `app.json` (`expo.version` + `android.versionCode`), not `build.gradle`. [^10][^11] (For the publish half itself, use **logos-publish-artifacts**.)

## 11. Release-build pre-flight — the three last-mile papercuts

These don't fail the JS/dev build; they fail `assembleRelease` **after** a 15-minute native compile (or, worse, ship a broken APK), so check them *before* kicking off the release build:

1. **Reusing a shared TS core needs a Metro `node:crypto` shim.** If the phone imports the same portable `packages/` the desktop uses (recommended — one fold everywhere), anything reaching for `node:crypto` (e.g. `randomUUID` for event ids) breaks the **release** bundle only, at the very last gradle step (`createBundleReleaseJsAndAssets`). Wire it up front: a `shims/crypto.js`, a `metro.config.js` `resolveRequest` that maps `node:crypto`/`crypto` to it (and add `expo-crypto`), a `babel.config.js`, and an `index.js` entry. The dev bundler is lenient; the release bundler is not.
2. **Config-plugin anchors are Expo-SDK-version-specific.** A `withXxx` plugin that string/regex-patches `MainApplication.kt` or `build.gradle` is written against one Expo template and silently no-ops or throws on another (e.g. the package-registration shape is `return PackageList(this).packages.apply { … }` on one SDK, a bare `return …` on another). **Verify each plugin's anchor against the file the *installed* Expo version actually emits**, and make the anchor tolerate both shapes — don't assume the template you wrote it against.
...
[^12]: liblogosdelivery kernel `mode` field — `getNodeInfo`/config help text: *"Kernel operating mode: Edge (client-only) or Core (full service node)"*, validation `expected 'Core' or 'Edge'`. Verified in the arm64 mobile `.so` and the LAN `delivery_module` (2026-08-09). Edge = filter-subscribe + lightpush-publish, no shard relay, no discv5 — the config-accepted light client, vs the raw `filter`/`lightpush` fields that make `waku_new` reject (§5). qaku mobile carries the Core-default + opt-in Edge toggle in `mobile/src/lib/logos-transport.ts` (`NODE_MODE`/`setNodeMode`, Edge config drops relay+discv5) and `sessions.ts` (persists `qaku-nodemode`, applies before `transport.start`). Provenance: memory `logos-delivery-edge-mode` — and its warning that Edge-on-cellular was the flaky path in the qaku "phone can't publish" saga (memory `qaku-publish-rng-root-cause`), so test on-device before defaulting.
