...
### Desktop PC/SC reality — the traps the optimistic version above hides (hardware-verified 2026-08)

The reader path is NOT "verifies like any signed event" out of the box. What `xAlisher/keycard-basecamp`'s module actually returns and needs:[^dtreal]

- **`checkSignStatus` returns a bare DER ECDSA signature** (`30 44 0220 r 0220 s`, ~70 bytes) — **no recovery id, no pubkey**. So (a) your compact-`r‖s` adapter must **parse DER first** (`d2i_ECDSA_SIG`, then left-pad r,s to 32 and low-S) — a naive first-64-bytes read is garbage; and (b) you **cannot recover the signer pubkey from one signature** (a single DER sig recovers to two candidates, no way to pick).
- **Enrolling the pubkey is the hard part, and the "obvious" paths are dead ends:** `checkAuthStatus` returns only the **32-byte X-coordinate** (no parity → unusable for an address); `deriveKey` fails **"No active sessions — authorize to derive keys"** (it needs a PIN session your app doesn't hold; the PIN lives in keycard-ui). **Solution: two-signature intersection** — sign TWO distinct enrol digests, recover the ≤2 candidate pubkeys from each (feed a synthetic `r‖s‖v` for v∈{0,1} to your recover fn), and take the candidate **common to both**. Uniquely the card's key, no recovery id or session needed. Costs two taps at enrol.
- **Implicit unlock lives in `keycard-ui` (the module's own QML), not your app.** The desktop app never sees the PIN — keycard-ui calls `approveSign({signId, pin})`. Cache the PIN there (session-only, cleared on wrong PIN), auto-approve subsequent signs, and add an **auto-lock countdown + "Lock now"** so the user sees it's temporary. Enrol = 2 taps (+PIN each unless keycard-ui caches).
- **Identity binding must be enforced in the CORE, not the view.** Keycard is **opt-in per container** (an explicit bind), because loam correctly **suppresses a keycard *default*** for *unbound* containers (else every legacy calendar demands a tap). So on create/join the **core** must bind the chosen identity, or the current default when the view passes nothing — Basecamp serves **stale cached QML** (`~/.cache/Logos/LogosBasecamp/qmlcache/*`), so a view that "should" pass the identity often sends `''`. Trap: `getDefaultIdentityId()` returns a **JSON-quoted** string (`nlohmann::json(id).dump()`) — unwrap before binding or you bind a garbage id.
- **Rebuild/deploy gotchas:** rebuilding the keycard-ui `.lgx` with a **stale `logos-module-builder`** stamps `manifestVersion 0.2.0`, which current Basecamp **refuses to install** — and the warning it prints is a red herring (`Package is unsigned`; everything here is unsigned). Fix to **0.3.0**; `manifestVersion` is excluded from the Merkle root, so you can patch it in the manifest and repack (GNU tar) without recomputing hashes. Diagnose the whole chain by grepping the Basecamp log for your `fprintf(stderr,"[app] …")` lines.
...
[^dtreal]: Desktop PC/SC reality, verified against a real card 2026-08 driving `xAlisher/keycard-basecamp` from `loam_core` (0.4.0→0.4.3) + scala (0.9.1→0.9.5) + a forked `keycard-ui` (1.0.1/1.0.2): `checkSignStatus` → bare DER (siglen 70, `3044 0220…`); enrol via two-signature intersection (`derToRS`+`recoverCandidates` in `loam_identity.hpp`, `enrollKeycard` in `loam_core_impl.cpp`); `checkAuthStatus`→32B X, `deriveKey`→"No active sessions"; PIN cache + auto-lock added to `keycard-ui/qml/PinEntryScreen.qml`; core-side default binding + `getDefaultIdentityId` JSON-quote unwrap in scala `createCalendar`/`handleShareLink`; `manifestVersion 0.3.0` install gate + qmlcache staleness. Provenance: memories `keycard-enroll-sig-format`, `lgx-manifestversion-0-3-0`, `logos-lgx-hash-repack`.
