...
| Your OWN just-authored event vanishes on a fast app kill (publish → force-quit → reopen, it's gone — even though peers who received it still show it) | The local log save is **debounced** (e.g. 400 ms) for batching; a kill inside the window loses the append. It's a *persistence* bug, not sync — the network send raced ahead of the disk write | **Flush the authored event to disk synchronously BEFORE the network send.** Debounce is fine for remote ingests (re-delivered idempotently), but an event you created has no other copy until it's both on disk and on the wire — persist-then-publish, never publish-then-debounce.[^22] |
| A **fresh** node never meshes after the fleet was fine yesterday (existing long-lived nodes keep working; only new joins fail — "no mesh peer" on all shards) | The fleet **migrated clusters** under a stale preset: liblogosdelivery's baked preset still maps the fleet name to the OLD `clusterId`, so a new node dials the now-different-cluster boxes with mismatched config and never grafts a mesh. The **preset carries the cluster id** — a fleet can move clusters while its DNS name stays the same | Move to a preset that maps to the fleet's *current* cluster (e.g. `logos.dev`→cluster 3 broke joins → switch to `logos.test`, still cluster 2), keeping the entry-node multiaddrs pinned. Cross-check with an always-on hub: if IT still relays, the fleet's up and the failing client is the problem.[^23] |
