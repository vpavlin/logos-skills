...
**Channels are on the ui_qml Qt caller too — you do NOT need a Qt-free core to adopt SDS channels.** The channel methods are declared in the `delivery_module.lidl` (`channelCreate`/`channelSend`/`channelClose`/`channelExists` + the three `channelMessage*` events), and the builder generates them onto *both* bindings: the core-module `StdLogosResult` caller **and** the `ui_qml` C++-backend Qt caller. So a `ui_qml` module with a C++ backend can call `modules().delivery_module.channelCreate(channelId, contentTopic, senderId)` / `channelSend(channelId, payload)` and subscribe via `.on("channelMessageReceived", …)` directly — migrating a plain-relay ui_qml module to channels is an in-place backend edit, not a rewrite into a separate core. Recipe (proven to compile+link on the channels SDK — `delivery_module 0fb3a742…` / builder `afe4430…`): at each `subscribe(topic)` site also `channelCreate(topic, topic, senderId)`; switch `send(topic, sealed)` → `channelSend(topic, base64(sealed))` (the FFI base64s again = the double-base64 wire); register `.on("channelMessageReceived")` alongside `.on("messageReceived")` and keep a candidate-decoding receive (try raw / single- / double-base64) so relay AND channel wires both open. CAVEAT: "the Qt caller *exposes* channels" is build-verified; end-to-end interop still needs the on-device check (the five failure modes below apply identically).[^17]
...
[^17]: Verified 2026-08 building Perun's `perun_analytics` (a `ui_qml` C++-backend module) against the channels SDK: `modules().delivery_module.channelCreate/channelSend` + `.on("channelMessageReceived")` compiled and linked from the Qt caller (no Qt-free core), after bumping `delivery_module` to `0fb3a7427b29…` + builder `afe4430…`. Channel methods declared in `delivery_module.lidl` (`method channelCreate/channelSend/channelClose/channelExists`, `event channelMessageReceived/Sent/Error`), which the builder emits onto both the Std (core) and Qt (ui_qml) callers. Perun's relay→channels in-place migration + candidate-decoding receive: `perun/module/src/perun_analytics_backend.cpp` `ingestSealed`. End-to-end interop UNVERIFIED on-device at time of writing. Provenance: memory `perun-shared-transport-and-background`, `scala-calendar`.
